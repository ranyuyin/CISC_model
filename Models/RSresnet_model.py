...
def GFnet50(num_classes,
    ...
    img_input = layers.Input(shape=input_shape, batch_size=batch_size, name='gf')
    if rescale_inputs:
...
def GFnet50ann(num_classes,
    ...
    input_shape = (input_size, input_size, 3)
    img_input = layers.Input(shape=input_shape, batch_size=batch_size, name='gf')
    band_input = layers.Input(shape=(6), batch_size=batch_size, name='ldband')
    dem_input = layers.Input(shape=(3), batch_size=batch_size, name='dem')
    if rescale_inputs:
        # Hub image modules expect inputs in the range [0, 1]. This rescales these
        # inputs to the range expected by the trained model.
        x = layers.Lambda(
    ...
    ann_input = layers.Concatenate()([band_input,dem_input])
    x_ann = layers.Dense(n_hiden, activation='relu')(ann_input)

    x = layers.Concatenate()([x,x_ann])
    x = layers.Dense(
        num_classes,
        kernel_initializer=tf.initializers.random_normal(stddev=0.01),
        kernel_regularizer=_gen_l2_regularizer(use_l2_regularizer),
    ...
    # Create model.
    return tf.keras.Model([img_input,band_input,dem_input], x, name='resnet50ann')