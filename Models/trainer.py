...
import RSresnet_model
import yaml

def get_models() -> Mapping[str, tf.keras.Model]:
    """Returns the mapping from model type name to Keras model."""
    return {
        'gfldbanddem_resnetann':RSresnet_model.GFnet50ann,
    }
...
def define_classifier_flags():
    ...
    flags.DEFINE_string(
        'weight_df_name',
        default=None,
        help='The location of the weight df.')
    flags.DEFINE_string(
        'class_df_name',
        default=None,
        help='The location of the class df.')
...
def _get_dataset_builders(params: base_configs.ExperimentConfig,
    ...
    for config in dataset_configs:
        if config is not None and config.has_data:
            builder = RSdataset_factory.RSDatasetBuilder(
                config, 
                # weight_df_name=params.weight_df_name,
                # class_df_name=params.class_df_name,
...
def _count_dataset_shapefile(shp_path):
    gdf = gpd.read_file(shp_path)
    n_train = len(gdf.loc[gdf['split'] == 'train'])
    n_val = len(gdf.loc[gdf['split'] == 'val'])
...
class ReduceCategoricalAccuracy(tf.keras.metrics.Accuracy):
    def __init__(self, focusLabel, name='ReduceAccuracy', **kwargs) -> None:
        super().__init__(name=name, **kwargs)
        self.focusLabel = focusLabel

    def update_state(self, y_true, y_pred, sample_weight=None):
        y_pred = tf.argmax(y_pred, 1)
        y_pred = tf.reshape(tf.equal(y_pred,self.focusLabel),(-1,1))
        y_true = tf.reshape(tf.equal(y_true,self.focusLabel),(-1,1))
        super().update_state(y_true, y_pred, sample_weight)


def _get_metrics(one_hot: bool, **kwarg) -> Mapping[Text, Any]:
    ...
    if one_hot:
        ...
        return {
            ...
            'accuracy':
                tf.keras.metrics.SparseCategoricalAccuracy(name='accuracy'),
            'accreduce':
                ReduceCategoricalAccuracy(focusLabel=1),
        }
...
def train_and_eval(
    ...
    if params.runtime.num_gpus > 0:
        ...
        tf.config.set_visible_devices(physical_devices[params.runtime.skipgpu:], 'GPU')
    ...
    with strategy_scope:
        ...
        if params.train.resume_checkpoint:
            initial_epoch = resume_from_checkpoint(
                model=model, model_dir=params.model_dir, 
                train_steps= train_steps,
                # train_steps=1814,
                )
            if  params.train.resume_checkpoint_epoches is not None:
                initial_epoch = params.train.resume_checkpoint_epoches
